...
DO $$ BEGIN
    ...
    -- Try to add the foreign key separately to avoid blocking column creation
    DO $$ BEGIN
    ...
    -- Ensure sort_order exists (often used for display priority)
...
DO $$ BEGIN
    ...
    ALTER TABLE crm_prospects ADD COLUMN IF NOT EXISTS company VARCHAR(255);
...
END $$;
DO $$ BEGIN
  ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_view_homologation BOOLEAN DEFAULT false;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_delete_deals BOOLEAN DEFAULT false;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_delete_projects BOOLEAN DEFAULT false;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_delete_tasks BOOLEAN DEFAULT false;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
...
const step38PermissionTemplates = `
...
SELECT 'Vendedor', 'Chat, CRM básico, contatos e tarefas', 'UserCheck',
  '{"can_view_chat":true,"can_view_chatbots":false,"can_view_flows":false,"can_view_departments":false,"can_view_schedules":true,"can_view_tags":true,"can_view_contacts":true,"can_view_ai_secretary":false,"can_view_ai_agents":false,"can_view_crm":true,"can_view_prospects":true,"can_view_companies":true,"can_view_map":true,"can_view_calendar":true,"can_view_tasks":true,"can_view_reports":false,"can_view_revenue_intel":false,"can_view_ghost":false,"can_view_crm_settings":false,"can_view_projects":false,"can_view_campaigns":false,"can_view_sequences":false,"can_view_external_flows":false,"can_view_webhooks":false,"can_view_ctwa":false,"can_view_billing":false,"can_view_connections":false,"can_view_organizations":false,"can_view_settings":true}'::jsonb,
  true, 1
WHERE NOT EXISTS (SELECT 1 FROM permission_templates LIMIT 1);

INSERT INTO permission_templates (name, description, icon, permissions, is_default, sort_order)
SELECT 'Gerente', 'Tudo do vendedor + relatórios, projetos e departamentos', 'Briefcase',
  '{"can_view_chat":true,"can_view_chatbots":true,"can_view_flows":true,"can_view_departments":true,"can_view_schedules":true,"can_view_tags":true,"can_view_contacts":true,"can_view_ai_secretary":false,"can_view_ai_agents":true,"can_view_crm":true,"can_view_prospects":true,"can_view_companies":true,"can_view_map":true,"can_view_calendar":true,"can_view_tasks":true,"can_view_reports":true,"can_view_revenue_intel":true,"can_view_ghost":false,"can_view_crm_settings":true,"can_view_projects":true,"can_view_campaigns":true,"can_view_sequences":true,"can_view_external_flows":true,"can_view_webhooks":true,"can_view_ctwa":true,"can_view_billing":false,"can_view_connections":false,"can_view_organizations":false,"can_view_settings":true}'::jsonb,
  true, 2
WHERE (SELECT COUNT(*) FROM permission_templates) < 2;

INSERT INTO permission_templates (name, description, icon, permissions, is_default, sort_order)
SELECT 'Administrador', 'Acesso total a todos os módulos', 'Crown',
...
// ============================================
// STEP 40: MEETINGS MODULE
// ============================================
const step40Meetings = `
...
-- Documentos anexados a empresas de homologação
...
-- Unique constraint for upsert by org + order_number
...
-- Add logistics to organizations modules_enabled
UPDATE organizations
SET modules_enabled = modules_enabled || '{"logistics": false}'::jsonb
WHERE modules_enabled IS NOT NULL
  AND NOT (modules_enabled ? 'logistics');
...
END $$;
DO $$ BEGIN
  ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_delete_logistics BOOLEAN DEFAULT false;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
`;

const step57DealQuoteFields = `
DO $$ BEGIN
  ALTER TABLE crm_deals ADD COLUMN IF NOT EXISTS quote_carrier VARCHAR(255);
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE crm_deals ADD COLUMN IF NOT EXISTS quote_value NUMERIC(15,2) DEFAULT 0;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE crm_deals ADD COLUMN IF NOT EXISTS quote_code VARCHAR(100);
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
`;

const step58CnaeGroups = `
...
-- Permissões de acesso às tabelas de preço
...
CREATE INDEX IF NOT EXISTS idx_price_list_org ON price_lists(organization_id);
CREATE INDEX IF NOT EXISTS idx_price_list_items_list ON price_list_items(price_list_id);
CREATE INDEX IF NOT EXISTS idx_price_list_access_user ON price_list_access(user_id);
...
DO $$ BEGIN
  ...
  ALTER TABLE nfc_card_profiles ADD COLUMN IF NOT EXISTS showcase_image_url TEXT;
...
ALTER TABLE ead_courses ADD COLUMN IF NOT EXISTS has_certificate BOOLEAN DEFAULT true;
ALTER TABLE ead_courses ADD COLUMN IF NOT EXISTS passing_score INT DEFAULT 100;
...
ALTER TABLE ead_lessons ADD COLUMN IF NOT EXISTS module_id UUID REFERENCES ead_modules(id) ON DELETE SET NULL;
ALTER TABLE ead_lessons ADD COLUMN IF NOT EXISTS description TEXT;
