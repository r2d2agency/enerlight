...
async function generateReportText(orgId, userId, reportType, includeChannels, includeEnerlight) {
  ...
  const gd = {
    orcamento: { count: 0, value: 0, cost: 0, value_with_cost: 0 },
    pedido: { count: 0, value: 0, cost: 0, value_with_cost: 0 },
    faturamento: { count: 0, value: 0, cost: 0, value_with_cost: 0 },
  };
  ...
  try {
    ...
    try {
      ...
      carteiraValues = (cfg.rows[0]?.carteira_values || []).filter(Boolean);
    } catch (e) {
      console.error('[carteira-report] config indisponível:', e.message);
    }
