...
import { representantesApi, rpToken } from '@/lib/representantes-api';
...
const RepresentanteLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { token } = await representantesApi.login(result.data.email, result.data.password);
      rpToken.set(token);
      navigate('/representantes/dashboard', { replace: true });
    } catch (error) {
        ...
        <div className="w-full max-w-md space-y-6 min-w-0">
          <Card className="shadow-neon w-full overflow-hidden">
            <CardHeader className="text-center">
              ...
              <CardTitle>Portal de Representantes</CardTitle>
              <CardDescription>Acesse sua carteira de clientes e pedidos</CardDescription>
            </CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-right">
                    <Link to="/representantes/esqueci-senha" className="text-sm text-primary hover:underline">
                      Esqueci minha senha
                    </Link>
                  </div>
                </div>
...
export default RepresentanteLogin;
