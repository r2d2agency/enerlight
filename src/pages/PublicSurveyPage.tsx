    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map(i => (
        <button key={i} type="button" onClick={() => onChange(i)}>
          <Star className={`h-8 w-8 transition-colors ${i <= (value || 0) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`} />
        </button>
      ))}
    </div>
  );
        ...
        <Card>
          <CardContent className="pt-6 space-y-4">
            {survey.thumbnail_url && (
              <img src={survey.thumbnail_url} alt="" className="w-full h-48 object-cover rounded-lg" />
            )}
