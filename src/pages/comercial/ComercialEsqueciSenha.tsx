...
import { Loader2, MailCheck, ArrowLeft, Briefcase } from 'lucide-react';
import { z } from 'zod';

const emailSchema = z.string().trim().email({ message: 'Email inválido' });

const ComercialEsqueciSenha = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await comercialExternalApi.esqueciSenha(result.data);
      setSent(true);
    } catch (err) {
        ...
        <div className="w-full max-w-md space-y-6 min-w-0">
          <Card className="shadow-neon w-full overflow-hidden">
            ...
            <CardFooter className="justify-center pt-0">
              <Link to="/comercial/login" className="text-sm text-primary hover:underline flex items-center gap-1">
...
export default ComercialEsqueciSenha;
