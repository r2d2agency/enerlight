...
// Atende tanto /comercial/ativar-conta (convite) quanto /comercial/definir-senha (reset) —
// o back-end distingue os dois pelo campo `purpose` do próprio token.
const ComercialAtivarConta = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await comercialExternalApi.ativarConta(token, result.data.password);
  ...
  const description = purpose === 'reset'
    ? 'Defina uma nova senha para acessar o Portal Comercial.'
    : 'Defina sua senha para ativar o acesso ao Portal Comercial.';
          ...
          <Card className="shadow-neon w-full overflow-hidden">
            ...
            {checking ? (
              ...
              <CardContent className="space-y-4 text-center">
                ...
                <Link to="/comercial/esqueci-senha" className="text-sm text-primary hover:underline block">
                  Solicitar novo link
                </Link>
              </CardContent>
            ) : (
...
export default ComercialAtivarConta;
