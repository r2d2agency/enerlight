...
function MarketingContent({ internal = false }: { internal?: boolean }) {
  ...
  const previewType = (item: ComercialMarketingMaterial) => {
    const source = `${item.file_url || ''} ${item.original_name || ''} ${item.mime_type || ''}`.toLowerCase();
    const extension = source.split(/[?#]/)[0].split('.').pop() || '';
    if (item.material_type === 'link') return 'link';
    if (item.material_type === 'video' || item.mime_type?.startsWith('video/') || ['mp4', 'webm', 'ogg', 'ogv', 'mov', 'm4v'].includes(extension)) return 'video';
    if (item.material_type === 'image' || item.mime_type?.startsWith('image/') || ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'avif', 'tif', 'tiff', 'heic', 'heif', 'ico'].includes(extension)) return 'image';
    if (item.mime_type === 'application/pdf' || extension === 'pdf') return 'pdf';
    return item.file_url ? 'document' : 'other';
  };
  ...
  return <div className={internal ? 'min-h-[calc(100vh-3rem)] -m-4 space-y-6 bg-[#0A101A] p-4 text-[#F4F8FF] sm:-m-6 sm:p-6 lg:-m-7 lg:p-7' : 'space-y-6'}>
    ...
    <Dialog open={!!preview} onOpenChange={(open) => { if (!open) { setPreview(null); setPreviewError(false); } }}><DialogContent className="max-w-4xl"><DialogHeader><DialogTitle>{preview?.title}</DialogTitle></DialogHeader>{preview && <div className="space-y-4">{previewType(preview) === 'image' && !previewError && <img src={resolveMediaUrl(preview.file_url) || undefined} alt={preview.title} onError={() => setPreviewError(true)} className="max-h-[65vh] w-full object-contain" />}{previewType(preview) === 'image' && previewError && <div className="rounded border p-6 text-center"><p>Não foi possível carregar a imagem.</p><Button className="mt-3" onClick={() => download(preview)}>Baixar arquivo</Button></div>}{previewType(preview) === 'video' && !previewError && <video src={resolveMediaUrl(preview.file_url) || undefined} controls onError={() => setPreviewError(true)} className="max-h-[65vh] w-full" />}{previewType(preview) === 'video' && previewError && <div className="rounded border p-6 text-center"><p>O navegador não conseguiu reproduzir este vídeo.</p><Button className="mt-3" onClick={() => download(preview)}>Baixar arquivo</Button></div>}{previewType(preview) === 'pdf' && <iframe src={resolveMediaUrl(preview.file_url) || undefined} title={preview.title} className="h-[65vh] w-full" />}{previewType(preview) === 'document' && <iframe src={resolveMediaUrl(preview.file_url) || undefined} title={preview.title} className="h-[65vh] w-full" />}{previewType(preview) === 'link' && <div className="rounded border p-6 text-center"><p className="mb-3 text-muted-foreground">Este material é um link externo.</p><Button onClick={() => window.open(resolveMediaUrl(preview.file_url) || undefined, '_blank', 'noopener,noreferrer')}>Abrir link</Button></div>}{previewType(preview) === 'other' && <div className="rounded border p-6 text-center text-muted-foreground">Arquivo indisponível para prévia.</div>}{['document', 'other'].includes(previewType(preview)) && <div className="rounded border p-5"><p className="font-medium">Prévia não disponível no navegador</p><p className="mt-1 text-sm text-muted-foreground">{preview.original_name || preview.title}{formatBytes(preview.file_size) ? ` · ${formatBytes(preview.file_size)}` : ''}</p><Button className="mt-3" onClick={() => download(preview)}><Download className="mr-2 h-4 w-4" />Baixar arquivo</Button></div>}{preview.copy_text && <div><p className="mb-2 text-sm font-medium">Copy para publicação</p><div className="flex gap-2"><p className="flex-1 whitespace-pre-wrap rounded bg-muted p-4 text-sm">{preview.copy_text}</p><Button size="icon" variant="outline" onClick={() => copy(preview.copy_text)}><Copy className="h-4 w-4" /></Button></div></div>}</div>}</DialogContent></Dialog>
  </div>;
}
