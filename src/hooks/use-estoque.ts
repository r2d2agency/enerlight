...
export function useEstoque() {
  ...
  const importacoes = useQuery({ queryKey: ['estoque-importacoes'], queryFn: estoqueApi.listarImportacoes });
  const invalidate = () => { ['estoque-produtos', 'estoque-movimentos', 'estoque-alertas', 'estoque-importacoes'].forEach(key => client.invalidateQueries({ queryKey: [key] })); };
  ...
  return { produtos, movimentos, alertas, importacoes, movimento, produto, bom, consumirComposto, produzir, recarregar: invalidate };
}
