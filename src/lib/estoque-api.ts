...
export interface ImportarProdutosResultado { created: number; updated: number; errors: Array<{ row: number; error: string }> }
export interface ExcluirProdutosResultado { deleted: number; blocked: Array<{ id: string; sku?: string; name?: string; reason: string }> }
...
export const estoqueApi = {
  ...
  importarProdutos: (items: ProdutoInput[]) => api<ImportarProdutosResultado>('/api/stock/products/import', { method: 'POST', body: { items } }),
  excluirProdutos: (ids: string[]) => api<ExcluirProdutosResultado>('/api/stock/products/bulk-delete', { method: 'POST', body: { ids } }),
